...
pub async fn process_commit(
    ...
    process_commit_with_pr(subject, body, hash, author, include_pr, None, octocrab, owner, repo).await
}
...
pub fn consolidate_dependabot_updates(updates: Vec<String>) -> Vec<String> {
    let re_update = Regex::new(r"Updates `([^`]+)` from ([^ ]+) to ([^ ]+)(?: \((#[0-9]+)\))?").unwrap();
    let re_bump_link = Regex::new(r"Bumps? \[([^\]]+)\]\([^\)]+\) from ([^ ]+) to ([^ ]+)(?: \((#[0-9]+)\))?").unwrap();
    let re_bump_simple = Regex::new(r"Bumps? ([^ ]+) from ([^ ]+) to ([^ ]+)(?: \((#[0-9]+)\))?").unwrap();
    ...
    for line in updates {
        ...
        let pr_number = re_pr_number.captures(&line)
    ...
    for (pkg, (from, to, pr_numbers)) in package_updates {
        ...
        final_lines.push(format!("- Updates `{}` from {} to {}{}", pkg, from, to, pr_suffix));
    }
...
mod tests {
    ...
    async fn test_dependabot_with_body_include_pr() {
        let body = "Bumps [package]...\nUpdates `package` from 1.0 to 1.1\n...";
        let res = process_commit("Bump package (#123)", body, "sha", "dependabot[bot]", true, &None, "", "").await;
        assert_eq!(res, Some(ProcessedCommit::Dependabot(vec!["- Updates `package` from 1.0 to 1.1 (#123)".to_string()])));
    }
    ...
    async fn test_merge_pull_request_with_include_pr() {
        let res = process_commit("Merge pull request #123 from foo", "", "sha", "User", true, &None, "", "").await;
    ...
    async fn test_pr_number_added_when_missing() {
        ...
        assert_eq!(res, Some(ProcessedCommit::Other("- Fix important bug (User)".to_string())));
    }
    ...
    async fn test_pr_number_in_subject_preserved() {
        ...
        let res = process_commit("Bump software.amazon.awssdk:bom in the other-deps group (#2887)", "", "sha", "dependabot[bot]", true, &None, "", "").await;
        // PR number should be preserved in output
        assert_eq!(res, Some(ProcessedCommit::Dependabot(vec!["- Bump software.amazon.awssdk:bom in the other-deps group (#2887)".to_string()])));
    }

    #[tokio::test]
    async fn test_pr_number_from_merge_commit() {
        // Test that PR number from merge commit mapping is used
        let res = process_commit_with_pr("Bump the fiks-deps group across 1 directory with 8 updates", "", "sha", "dependabot[bot]", true, Some(2880), &None, "", "").await;
        // PR number should be added to output
        assert_eq!(res, Some(ProcessedCommit::Dependabot(vec!["- Bump the fiks-deps group across 1 directory with 8 updates (#2880)".to_string()])));
    }
    ...
    fn test_consolidate_preserves_pr_numbers() {
        ...
        assert!(res.iter().any(|line| line.contains("other") && line.contains("#2885")));
    }
